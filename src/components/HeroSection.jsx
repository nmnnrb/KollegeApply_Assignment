...
const HeroSection = () => {
  ...
  <svg  xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="lucide hover:translate-x-3 hover:animate-bounce animate-pulse transition duration-200 lucide-chevron-right">
    <path d="m9 18 6-6-6-6"/>
            ...
            <img onClick={() => navigate(`/post/${index}`)} src={post.image} className="w-full md:w-[750px] overflow-hidden h-[300px] md:h-[500px] object-cover rounded-t-lg" alt="HeroImage" />
            ...
            <h1 onClick={() => navigate(`/post/${index}`)} className='text-[#02234D] max-w-full md:max-w-[750px] break-words font-bold py-[5px] px-[22px] text-[24px] md:text-[30px] mt-2'>{post.title}</h1>
            <p className='text-[#40347D] px-[22px] py-[5px] text-xs'>{new Date(post.publishedAt).toLocaleString()}</p>
            <p onClick={() => navigate(`/post/${index}`)} className='text-[#40347D] max-w-full md:max-w-[750px] px-[22px] py-[5px] text-xs md:text-sm'> {
                                post.description.split(" ").length >200 ?  post.description.split(" ").slice(0, 90).join(" ") + "..." : post.description
                                    }</p>
        <p onClick={() => navigate(`/post/${index}`)}   className='text-[#40347D] max-w-full md:max-w-[750px] px-[22px] py-[5px] hover:text-blue-600 text-end hover:font-bold transition duration-300 text-xs md:text-sm'  >Read More...</p>
